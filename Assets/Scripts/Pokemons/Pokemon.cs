...
public class Pokemon
{
    ...
    public void Init()
    {
        ...
        if (Base.AbilityID != AbilityID.none)
            Ability = AbilityDB.Ablities[Base.AbilityID];
        ResetBoost();
    ...
    public float ModifyAtk(float atk,Pokemon defender,Move move)
    {
        if (Ability?.OnModifyAttack != null)
            return Ability.OnModifyAttack(atk, this, defender, move);

        return atk;
    }

    public float ModifySpAtk(float atk, Pokemon defender, Move move)
    {
        if (Ability?.OnModifySpAttack != null)
            return Ability.OnModifySpAttack(atk, this, defender, move);
